    ...
    <script>
            ...
            $("#table_list").bootstrapTable({
                ...
                columns: [{
                    ...
                    formatter: function(value,row,index) {
                        // return '<span style="color:'+ value +'" class="label label-danger"></span>';
                        return '<span style="background-color:'+ value +';" class="label label-primary">标签</span>';
                    }
                },{
                    title: "创建时间",
                    field: "createTime",
        ...
        function edit(id){
            layer.open({
                ...
                shade: false,
                area: ['893px', '600px'],
                content: '/admin/productType/form?id=' + id,
                end: function(index){
        ...
        function add(){
            layer.open({
                ...
                shade: false,
                area: ['893px', '600px'],
                content: '/admin/productType/form',
                end: function(index) {
                    $('#table_list').bootstrapTable("refresh");
                }
            });
        ...
        function del(id) {
            layer.confirm('确定删除吗?', {icon: 1, title:'提示'}, function(index){
                $.ajax({
                    type: "POST",
                    dataType: "json",
                    url: "${ctx!}/admin/productType/delete/" + id,
                ...
                layer.confirm('确定删除吗?', {icon: 1, title:'提示'}, function(index){
                    $(items).each(function (index, item) {
                        $.ajax({
                            type: "POST",
                            dataType: "json",
                            url: "${ctx!}/admin/productType/delete/" + item.id,
                            success: function(msg){
